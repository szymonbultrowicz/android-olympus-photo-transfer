import { retrieveDirectories } from './html-response-parser';
...
wlansd = new Array();
wlansd[0]="/DCIM,100OLYMP,0,16,19163,26163";
wlansd.sort(cmptime);
wlansd.sort(cmpdate);
wlansd.reverse();
...
describe('retrieveDirectories', () => {
    it('should find a directory', () => {
        const result = retrieveDirectories(HTML);

        expect(result).toEqual(['100OLYMP']);
    });
});
